...
public class NamingService {
	...
	public Pager findAll(Object object, Pager pager, Conference selectedConference, int tableLength, int buttonsToShow, Pageable pageable) {
		if (object instanceof TableAbstract) {
			Page<TableAbstract> tablePage = abstractService.findAllByConference(selectedConference, pageable).map(abstractMapper::toTableAbstract);
			...
			Page<TableAlarm> tablePage = alarmService.findAllByConference(selectedConference, pageable).map(alarmMapper::toTableAlarm);
			List<Object> list = !tablePage.getContent().isEmpty() ? new ArrayList<Object>(tablePage.getContent()) : new ArrayList<Object>();
			if(tableLength > list.size()) {
			...
			Page<TableEvent> tablePage = eventService.findAllByConference(selectedConference, pageable).map(eventMapper::toTableEvent);
			List<Object> list = !tablePage.getContent().isEmpty() ? new ArrayList<Object>(tablePage.getContent()) : new ArrayList<Object>();
			if(tableLength > list.size()) {
			...
			Page<TableGrade> tablePage = gradeService.findAllByConference(selectedConference, pageable).map(gradeMapper::toTableGrade);
			List<Object> list = !tablePage.getContent().isEmpty() ? new ArrayList<Object>(tablePage.getContent()) : new ArrayList<Object>();
			if(tableLength > list.size()) {
